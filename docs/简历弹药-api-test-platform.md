...
一个自用接口回归与监控工具：存用例 → 手动 / 定时跑 → 多维度断言（状态码 / 包含 / 耗时 / JSONPath）→ 出报告 → 带账号体系 → 一键容器化部署。技术栈 Fastify5 + Node 内置 SQLite + Vue3/Element Plus + node-cron，**113 测试全绿**。**配套一个「被测系统」office-oa，用这个平台把它测穿了。**
