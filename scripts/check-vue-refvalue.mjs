...
for (const f of files) {
  ...
  // 箭头函数形参也算声明。少了这一段会把 `.map((j) => ...j.value...)` 里的
  // `j.value` 误报成「未声明的 ref」—— 但这里的 j 是普通对象，.value 只是它的**属性**，
  // 跟 ref 无关。（office-oa 那版没这段，因为它的视图里没有这种写法，一换项目就冒出来了。）
  for (const mm of code.matchAll(/\(([^()]*)\)\s*(?:=>|\{)/g)) {
    mm[1].split(',').forEach((s) => {
      const n = s.trim().split(/\s*[:=]\s*/)[0].replace(/^\.\.\./, '').trim()
      if (/^[A-Za-z_$][\w$]*$/.test(n)) declared.add(n)
    })
  }
  for (const mm of code.matchAll(/\b([A-Za-z_$][\w$]*)\s*=>/g)) declared.add(mm[1])
