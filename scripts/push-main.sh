...
#   3) curl 的探测结果**不能**用来判断通道通不通：今天实测出现过
#      「探测 200 但 push 报 Recv failure」「探测 000 但 push 能成」两种。
#      所以本脚本不预筛，直接按顺序 push，**推送后问远端 sha**，
#      与本地 HEAD 一致才算成功，否则换下一条通道。
...
try_channel() {  # $1 = 代理地址（空串 = 直连）；成功返回 0
  ...
  # 不做 curl 预筛：今天实测「探测 200 但 push reset」和「探测 000 但 push 能成」
  # 两种都出现过，所以直接推，让 push 自己的 timeout 兜底。
