...
for (const f of files) {
  ...
  lines.forEach((line, i) => {
    for (const m of line.matchAll(/\b([A-Z][\w$]*)\b/g)) {
      ...
      // 跳过对象字面量的**键**：`return { GET: 'a', POST: 'b' }[m]` 里的 GET/POST 是键，不是引用。
      // 判据必须同时卡两头：前面是 `{` 或 `,`，后面跟 `:`。
      // 只卡「后面有冒号」是不行的——三元 `x ? Foo : bar` 的 Foo 后面也是冒号，那是真引用，
      // 误跳过它就等于漏掉「Foo 没声明」这个真 bug（这类 bug build 只 warning，真机才炸）。
      if (/[{,]\s*$/.test(line.slice(0, idx)) && /^\s*:/.test(line.slice(idx + name.length))) continue
