...
> 面向全栈 / 测试运维岗位的简历项目。**M4 已完成**：在 M3（JSONPath 断言 + 定时任务 + 报告）基础上新增 **JWT 登录鉴权 + 路由守卫 + 前端登录页**。
...
- **默认账号**：服务首次启动（库里无用户时）自动创建 `admin / admin123`，请尽快修改密码（当前未提供改密接口，M5 可补）。
...
| **M5** | 部署（Vercel / 云服务器）+ 改密接口（有余力） | ⏳ |

## 目录

```
index.js          Fastify 应用 + 路由（含 auth/schedules/runs/reports）+ onRequest 鉴权守卫
src/db.js         SQLite 封装（test_cases / runs / schedules / users）
src/auth.js       鉴权核心：scrypt 密码哈希 + 手写 HS256 JWT（signToken/verifyToken）
src/users.js      用户服务：createUser/verifyLogin/initDefaultUser
...
tests/app.test.js + tests/jsonpath.test.js + tests/auth.test.js  共 37 例，全离线
